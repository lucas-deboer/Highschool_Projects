...
class Controller():
    ...
    def update(self):
        ...
        for i in range(4): # THIS VALUE NEEDS TO BE >= THE MAX NUMBER OF BUTTONS
    ...
    # Translate input to button id. This supports strings or numbers (so people can code with whatever is easiest)
    def to_button_id(self, input):
        if input in range(6):  # THIS ALSO NEEDS TO BE >= THE TOTAL NUMBER OF BUTTONS
            return input
        else:
            print(
                "Invalid input. Input controller value must be between 0-3, or \"a\", \"b\", \"x\", \"y\", \"player\", or \"menu\".")
            return None
